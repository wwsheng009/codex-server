...
export type ListTurnPolicyDecisionsInput = {
  threadId?: string;
  policyName?: string;
  action?: string;
  actionStatus?: string;
...
export type GetTurnPolicyMetricsInput = {
  threadId?: string;
  source?: string;
};

export type ListHookRunsInput = {
  runId?: string;
  threadId?: string;
  eventName?: string;
  status?: string;
...
export function listThreadsPage(
  ...
  return apiRequest<ThreadListPage>(
    `/api/workspaces/${workspaceId}/threads${suffix}`,
  );
}

export function listLoadedThreadIds(workspaceId: string) {
  return apiRequest<string[] | { data?: unknown }>(
    `/api/workspaces/${workspaceId}/threads/loaded`,
  ).then((result) => {
...
export function getThread(
  ...
  return apiRequest<ThreadDetail>(
    `/api/workspaces/${workspaceId}/threads/${threadId}${suffix}`,
  );
}
...
export function listTurnPolicyDecisions(
  workspaceId: string,
  input: ListTurnPolicyDecisionsInput = {},
) {
  const query = new URLSearchParams();
  if (input.threadId) {
    query.set("threadId", input.threadId);
  }
  if (input.policyName) {
  ...
  const suffix = query.size ? `?${query.toString()}` : "";
  return apiRequest<TurnPolicyDecision[]>(
    `/api/workspaces/${workspaceId}/turn-policy-decisions${suffix}`,
  );
}

export function listHookRuns(
  workspaceId: string,
  input: ListHookRunsInput = {},
) {
  const query = new URLSearchParams();
  if (input.runId) {
  ...
  if (input.eventName) {
    query.set("eventName", input.eventName);
  }
  if (input.status) {
    query.set("status", input.status);
  }
  if (input.handlerKey) {
    query.set("handlerKey", input.handlerKey);
  }
  if (input.limit && input.limit > 0) {
    query.set("limit", String(input.limit));
  }
  ...
  const suffix = query.size ? `?${query.toString()}` : "";
  return apiRequest<HookRun[]>(`/api/workspaces/${workspaceId}/hook-runs${suffix}`);
}

export function getTurnPolicyMetrics(
  workspaceId: string,
  input: GetTurnPolicyMetricsInput = {},
) {
  const query = new URLSearchParams();
  if (input.threadId) {
    query.set("threadId", input.threadId);
  ...
  const suffix = query.size ? `?${query.toString()}` : "";
  return apiRequest<TurnPolicyMetricsSummary>(
    `/api/workspaces/${workspaceId}/turn-policy-metrics${suffix}`,
  );
}
...
export function resumeThread(workspaceId: string, threadId: string) {
  return apiRequest<Thread>(
    `/api/workspaces/${workspaceId}/threads/${threadId}/resume`,
    {
      method: "POST",
    },
  );
...
export function renameThread(
  ...
  return apiRequest<Thread>(
    `/api/workspaces/${workspaceId}/threads/${threadId}/name`,
    {
      method: "POST",
      body: JSON.stringify(input),
    },
...
export function archiveThread(workspaceId: string, threadId: string) {
  return apiRequest<Thread>(
    `/api/workspaces/${workspaceId}/threads/${threadId}/archive`,
    {
      method: "POST",
    },
  );
...
export function unarchiveThread(workspaceId: string, threadId: string) {
  return apiRequest<Thread>(
    `/api/workspaces/${workspaceId}/threads/${threadId}/unarchive`,
    {
      method: "POST",
    },
  );
...
export function compactThread(workspaceId: string, threadId: string) {
  return apiRequest<{ status: string }>(
    `/api/workspaces/${workspaceId}/threads/${threadId}/compact`,
    {
      method: "POST",
    },
  );
