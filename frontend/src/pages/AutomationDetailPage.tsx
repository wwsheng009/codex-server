      ...
      <PageHeader
        actions={
          <div className="header-actions">
            ...
            {automation.jobId ? (
              <Button onClick={() => navigate(`/jobs?jobId=${encodeURIComponent(automation.jobId ?? '')}`)}>
                {i18n._({ id: 'Open Linked Job', message: 'Open Linked Job' })}
              </Button>
            ) : null}
            <Button
              intent="secondary"
          ...
          <div className="automation-meta-group">
            ...
            <span className="meta-pill">
              {i18n._({
                ...
                values: { schedule: formatScheduleLabel(automation.schedule) },
              })}
            </span>
            <span className="meta-pill">
              {i18n._({
          ...
          {automation.managedBy === 'background_job' ? (
            <>
              <div className="section-header">
                <div>
                  <h2>{i18n._({ id: 'Execution Backend', message: 'Execution Backend' })}</h2>
                  <p>
                    {i18n._({
                      id: 'This automation is scheduled and executed by the unified background job framework.',
                      message: 'This automation is scheduled and executed by the unified background job framework.',
                    })}
                  </p>
                </div>
              </div>
              <div className="detail-list">
                <DetailRow
                  label={i18n._({ id: 'Managed By', message: 'Managed By' })}
                  value={i18n._({ id: 'Background Jobs', message: 'Background Jobs' })}
                />
                <DetailRow
                  label={i18n._({ id: 'Linked Job ID', message: 'Linked Job ID' })}
                  value={automation.jobId ?? '—'}
                />
                <DetailRow
                  label={i18n._({ id: 'Job Status', message: 'Job Status' })}
                  value={<StatusPill status={automation.jobStatus || automation.status} />}
                />
                <DetailRow
                  label={i18n._({ id: 'Executor', message: 'Executor' })}
                  value={automation.jobExecutor || '—'}
                />
                <DetailRow
                  label={i18n._({ id: 'Last Run Status', message: 'Last Run Status' })}
                  value={automation.lastRunStatus || '—'}
                />
              </div>
            </>
          ) : null}
...
function DetailRow({ label, value }: AutomationDetailRowProps) {
  return (
    <div className="detail-row">
      <span>{label}</span>
      {typeof value === 'string' || typeof value === 'number' ? <strong>{value}</strong> : <div>{value}</div>}
    </div>
  )
}
